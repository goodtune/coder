...
func (a *agent) runLoop() {
	go a.reportLifecycleLoop()
	go a.reportMetadataLoop()
	go a.manageProcessPriorityLoop()

	// need to keep retrying up to the hardCtx so that we can send graceful shutdown-related
	// messages.
	for retrier := retry.New(100*time.Millisecond, 10*time.Second); retrier.Wait(a.hardCtx); {
		a.logger.Info(a.hardCtx, "connecting to coderd")
		err := a.run()
		// Cancel after the run is complete to clean up any leaked resources!
		if err == nil {
			continue
		}
		if a.hardCtx.Err() != nil {
		...
		if errors.Is(err, io.EOF) {
			a.logger.Info(a.hardCtx, "disconnected from coderd")
			continue
		}
		a.logger.Warn(a.hardCtx, "run exited with error", slog.Error(err))
	}
}
...
func (a *agent) reportMetadataLoop() {
...
func (a *agent) reportLifecycleLoop() {
...
func (a *agent) run() (retErr error) {
	...
	arm := newAPIRoutineManager(a.gracefulCtx, a.hardCtx, a.logger, conn)

	arm.run("init service banner", gracefulShutdownBehaviorStop,
	...
	arm.run("handle manifest", gracefulShutdownBehaviorStop, a.handleManifest(manifestOK))

	arm.run("app health reporter", gracefulShutdownBehaviorStop,
		func(ctx context.Context, conn drpc.Conn) error {
			select {
			case <-ctx.Done():
				return nil
	...
	arm.run("create or update network", gracefulShutdownBehaviorStop,
		a.createOrUpdateNetwork(manifestOK, networkOK))

	arm.run("coordination", gracefulShutdownBehaviorStop,
		func(ctx context.Context, conn drpc.Conn) error {
			select {
			case <-ctx.Done():
				return nil
	...
	arm.run("derp map subscriber", gracefulShutdownBehaviorStop,
		func(ctx context.Context, conn drpc.Conn) error {
			select {
			case <-ctx.Done():
				return nil
	...
	arm.run("fetch service banner loop", gracefulShutdownBehaviorStop, a.fetchServiceBannerLoop)

	arm.run("stats report loop", gracefulShutdownBehaviorStop, func(ctx context.Context, conn drpc.Conn) error {
		select {
		case <-ctx.Done():
			return nil
		case <-networkOK:
...
func (a *agent) runCoordinator(ctx context.Context, conn drpc.Conn, network *tailnet.Conn) error {
	...
	go func() {
		...
		case <-ctx.Done():
			...
			if err != nil {
				a.logger.Error(ctx, "failed to close remote coordination", slog.Error(err))
			}
			return
...
func (a *agent) manageProcessPriorityLoop() {
...
func newAPIRoutineManager(gracefulCtx, hardCtx context.Context, logger slog.Logger, conn drpc.Conn) *apiRoutineManager {
...
func (a *apiRoutineManager) run(name string, b gracefulShutdownBehavior, f func(context.Context, drpc.Conn) error) {
...
func (a *apiRoutineManager) wait() error {
	return a.eg.Wait()
}
